...
- [ ] **3.1 `data/loader.ts`** — OHLCV 적재 인터페이스. 소스 추상화(CSV/외부 API). 기준 지수(`^GSPC`/`SPY`), 보조(`^VIX`/`^VIX3M`), 유니버스 종목 공급
- [ ] **3.2 시점별 유니버스 공급** — `strategy-pool.md` 6절. **생존편향 차단**: 그 시점에 실제 존재·편입된 종목만. point-in-time 데이터 없으면 **섹터 ETF 유니버스**로 시작(편향 축소)
- [ ] **3.3 데이터 정합 검사** — 결측·중복 타임스탬프·정렬 검증 유틸. 분할(splitter는 Phase 2.6)
- [ ] **3.4 백테스트 데이터셋 캐시** — 로컬 캐시(반복 백테스트 속도). 토스 `/candles` 깊이 부족 시 외부 소스로 보강하는 분리 전략 메모(`execution-and-data.md` 6절 미정3)
- [ ] **3.5 단위 테스트** — 로더가 PriceSeries를 시간순 정렬·결측 없이 반환
